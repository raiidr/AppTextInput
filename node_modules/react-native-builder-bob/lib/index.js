...
_yargs.default.command('init', 'configure the package to use bob', {}, async () => {
  ...
  if (argv.target != null) {
    buildTarget(argv.target, report, source, output, exclude);
  } else {
    for (const target of options.targets) {
      buildTarget(target, report, source, output, exclude);
    }
  }
}).demandCommand().recommendCommands().strict().argv;
async function buildTarget(target, report, source, output, exclude) {
